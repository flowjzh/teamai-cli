...
const { resolvePostPullSpec, runPostPull } = await import('../post-pull.js');
const { DEFAULT_POST_PULL_TIMEOUT_SEC } = await import('../types.js');
...
describe('resolvePostPullSpec', () => {
  ...
  it('rejects a path that escapes the clone', () => {
    // no file needed: the traversal itself is what gets rejected
...
describe('runPostPull (supervisor)', () => {
  ...
  it('kills a script that overruns its deadline', async () => {
    ...
    expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining('postPull: timed out after 1s'));
